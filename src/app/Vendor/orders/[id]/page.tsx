...
export default function VendorOrderDetailPage({ params }: Props) {
  ...
  const { data: order, isLoading, refetch } = api.order.getById.useQuery({ id });
  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button asChild variant="outline" size="sm" className="mt-3"><Link href="/Vendor/orders">Go back</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-6">
      <Button asChild variant="outline" size="sm" className="border-black/10">
        <Link href="/Vendor/orders"><ArrowLeftIcon className="mr-1.5 h-4 w-4" /> Back</Link>
      </Button>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label>Reason *</Label>
              <Input placeholder="Explain why..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} />
            </div>
            <Button className="w-full bg-red-500 text-white hover:bg-red-600" disabled={!rejectReason.trim() || respond.isPending} onClick={() => respond.mutate({ id: order.id, action: "rejected", rejectReason })}>
              Reject Order
            </Button>
          </div>
        </DialogContent>
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {order.status === "pending" && (
                  <>
                    <Button size="sm" className="bg-green-600 text-white hover:bg-green-700" disabled={respond.isPending} onClick={() => respond.mutate({ id: order.id, action: "accepted" })}>
                      <CheckIcon className="mr-1 h-3.5 w-3.5" /> Accept
                    </Button>
                    <Button size="sm" variant="outline" className="border-red-200 text-red-500 hover:bg-red-50" onClick={() => setRejectOpen(true)}>
                      <XIcon className="mr-1 h-3.5 w-3.5" /> Reject
                    </Button>
                  </>
                )}
                {order.status === "accepted" && (
                  <Button size="sm" className="bg-purple-600 text-white hover:bg-purple-700" disabled={updateStatus.isPending} onClick={() => updateStatus.mutate({ id: order.id, status: "in_progress" })}>
                    Start Progress
                  </Button>
                )}
                {order.status === "in_progress" && (
                  <Button size="sm" className="bg-green-600 text-white hover:bg-green-700" disabled={updateStatus.isPending} onClick={() => updateStatus.mutate({ id: order.id, status: "delivered" })}>
                    Mark Delivered
                  </Button>
                )}
              </div>
